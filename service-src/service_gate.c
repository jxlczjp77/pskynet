...
#define vsnprintf sn_vsprintf
#define snprintf sn_sprintf
...
static void
_ctrl_(struct gate * g, const void * msg, int sz, char *tmp) {
	struct skynet_context * ctx = g->ctx;
	memcpy(tmp, msg, sz);
	tmp[sz] = '\0';
	char * command = tmp;
...
gate_init(struct gate *g , struct skynet_context * ctx, char * parm) {
	...
	char binding[1024];
	if (sz > 1024) {
		skynet_error(ctx, "Too long parm %s", parm);
		return 1;
	}
#else
	char watchdog[sz];
